    ...
    const loadStall = async () => {
      try {
        const [stallResponse, menuResponse] = await Promise.all([
          api.get(`/marketplace/stalls/${stallId}`),
          api.get(`/marketplace/stalls/${stallId}/menu`),
        ]);
        const stallData = stallResponse.data?.data ?? stallResponse.data;
        const menuData = Array.isArray(menuResponse.data?.data) ? menuResponse.data.data : (Array.isArray(menuResponse.data) ? menuResponse.data : []);
        
        setStall(normalizeStall(stallData));
        setMenuItems(menuData);
      } catch (error) {
        console.error("Failed to load stall details:", error);
  ...
  const handleConfirmOrder = async () => {
    try {
      ...
      await api.post('/orders', {
        ...
        specialInstructions: `Payment: ${paymentMethod}`
      });
        ...
        {menuItems.length > 0 ? menuItems.map((item) => (
          <div key={item.id} style={{ backgroundColor: "white", borderRadius: "20px", overflow: "hidden", border: "1px solid #e2e8f0", boxShadow: "0 4px 12px rgba(15,23,42,0.06)" }}>
            ...
            <div style={{ padding: "1.25rem" }}>
              ...
              <button
                disabled={orderingId === item.id || !stall.isOpen}
                onClick={() => handleBuy(item.id)}
                style={{ width: "100%", padding: "0.8rem", border: "none", borderRadius: "12px", backgroundColor: stall.isOpen ? "#7A0019" : "#94a3b8", color: "white", fontWeight: 700, cursor: stall.isOpen ? "pointer" : "not-allowed" }}
              >
          ...
          <div style={{ gridColumn: "1 / -1", padding: "2rem", backgroundColor: "white", borderRadius: "20px", textAlign: "center", color: "#64748b" }}>
          ...
          <div style={{
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              <input
                ...
                max="50"
                value={quantity}
                onChange={(e) => setQuantity(Math.min(Math.max(e.target.value, 1), 50))}
                style={{
                  width: "100%",
                  padding: "0.75rem",
